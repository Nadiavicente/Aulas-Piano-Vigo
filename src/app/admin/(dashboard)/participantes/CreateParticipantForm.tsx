...
export function CreateParticipantForm({ rounds }: { rounds: Round[] }) {
  ...
  return (
    <div>
        ...
        <form
          action={(fd) => {
            ...
            startTransition(async () => {
              ...
              setResult(res.ok ? { password: res.password } : { error: res.error });
            });
          }}
      ...
      {result?.password && (
        <p className="mt-2 rounded-md bg-slot-free/10 px-3 py-2 text-sm text-ink">
          Participante creado. Contraseña generada: <strong>{result.password}</strong> — reenvíasela
          manualmente por correo.
        </p>
      )}
      {result?.error && (
